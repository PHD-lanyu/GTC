...
def load_data(data_name, t_hops=7, data_dir='../data/',start_hop=0, cache_sub_dir='cache'):
    ...
    if data_name == 'acm':  # the acm dataset
        ...
        if os.path.exists(cache_dir):
            ...
            pos = ((sparse_mx_to_torch_sparse_tensor(apa).to_dense()
                    + sparse_mx_to_torch_sparse_tensor(
                        apcpa).to_dense() + sparse_mx_to_torch_sparse_tensor(aptpa).to_dense()) >= meta_path_num).float()
        ...
        if os.path.exists(cache_dir):
            ...
            pap_src_node = mam.row
            prp_src_node = mdm.row
            mwm_src_node = mwm.row
            pap_dst_node = mam.col
            prp_dst_node = mdm.col
            mwm_dst_node = mwm.col
            graph_data = {
                ('movie', 'net_mam', 'movie'): (torch.tensor(pap_src_node), torch.tensor(pap_dst_node)),
                ('movie', 'net_mdm', 'movie'): (torch.tensor(prp_src_node), torch.tensor(prp_dst_node)),
            ...
            pos = ((sparse_mx_to_torch_sparse_tensor(mam).to_dense()
                    + sparse_mx_to_torch_sparse_tensor(
                        mdm).to_dense() + sparse_mx_to_torch_sparse_tensor(mwm).to_dense()) >= meta_path_num).float()
